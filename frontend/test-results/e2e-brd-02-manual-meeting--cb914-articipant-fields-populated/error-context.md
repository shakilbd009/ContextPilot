...
- Name: e2e/brd-02-manual-meeting-import.spec.ts >> AC-07: Save succeeds with all optional participant fields populated
- Location: tests/e2e/brd-02-manual-meeting-import.spec.ts:239:1
...
Locator: getByText('Full Participant Details Meeting')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('Full Participant Details Meeting')
...
- main:
  ...
  - textbox "Meeting title":
    ...
    - text: Full Participant Details Meeting
  - text: Completed date
  ...
  - group "Participants":
    ...
    - textbox "Organization (optional)":
      ...
      - text: Acme Corp
    - text: Role / title (optional)
    - textbox "Role / title (optional)":
      - /placeholder: Engineer
      - text: Engineer
    - button "Add participant"
    - button "Hide advanced participant details" [expanded]
...
```ts
  164 |   });
  165 |   // Clear title
  166 |   await page.getByLabel(/meeting title/i).clear();
  167 |   await submitForm(page);
  168 |   // Should show field error for title
  169 |   const errorMsg = page.getByText(/title/i).first();
  170 |   await expect(errorMsg).toBeVisible();
  171 |   // Form should stay on /meetings/new
  172 |   await expect(page).toHaveURL(/\/meetings\/new/);
  173 | });
  174 | 
  175 | // ── AC-04 ──────────────────────────────────────────────────────────
  176 | 
  177 | test('AC-04: Save blocked when completed date/time is missing', async ({ page }) => {
  178 |   await signIn(page);
  179 |   await page.goto('/meetings/new');
  180 |   await fillForm(page, {
